...
class PodcastViewHolder(itemView: View, private val mDelegate: PodcastDelegate) :
    ...
    init {
        ...
        itemView.download_img.setOnClickListener {
            mData?.let {
                it?.let { mdata -> mDelegate.onTapDownloadPodcastItem(mdata) }
            }
        }
    }
    ...
    override fun bindData(data: EpisodeVO) {
        mData = data
        mData?.image?.let { itemView.podcast_img.load(it) }
        itemView.podcast_hour.text = mData?.audio_length_sec?.toLong()?.convertMillisecondToHMS()
        itemView.description.text = mData?.title
    }
}